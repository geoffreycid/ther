...
def evaluate(dict_env, dict_agent, dict_expert, policy_net, net_expert, writer, global_step, memory):


    # Agent's policy

    accuracy_policy = 0
    batch_imcs = memory.imc(*zip(*memory))
    batch_states = torch.cat(batch_imcs.state)
    policy_net.prediction_mission(batch_states)
    # Expert
    accuracy_expert = 0
    batch_imcs = memory.imc(*zip(*memory))
    batch_states = torch.cat(batch_imcs.state)[:, 12:]
    pred_sentences = net_expert.prediction_mission(batch_states)
    sorted_predictions = torch.sort(pred_sentences[:, -4:], axis=1)[0]
    sorted_ground_truth = torch.sort()
    if "learned" in dict_expert["name"]:
