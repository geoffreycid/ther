...
def evaluate(dict_env, dict_agent, policy_net, net_expert, writer, global_step):
    ...
    writer.add_scalar("Mean reward in {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
                      reward_smoothing / dict_agent["max_steps_evaluate"], global_step=global_step)
    writer.add_scalar("Mean discounted reward in {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
                      discounted_reward_smoothing / dict_agent["max_steps_evaluate"], global_step=global_step)
    writer.add_scalar("Mean length episode during {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
                      length_episode_done_smoothing / episode_done_smoothing, global_step=global_step)
    writer.add_scalar("Mean reward per episode during {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
                      reward_smoothing / episode_done_smoothing, global_step=global_step)
    writer.add_scalar("Timeout per episode during {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
                      timeout_smoothing / episode_done_smoothing, global_step=global_step)
    writer.add_scalar("Object picked per episode during {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
                      object_picked_smoothing / episode_done_smoothing, global_step=global_step)
    writer.add_scalar("Success rate per episode during {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
                      success_rate_smoothing / episode_done_smoothing, global_step=global_step)

    if not dict_env["wrong_object_terminal"]:
        writer.add_scalar("Wrong object picked rate per episode during {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
    ...
    if use_learned_expert:
        writer.add_scalar("Mean acc of pred per episode during {} steps on unseen instructions".format(dict_agent["max_steps_evaluate"]),
