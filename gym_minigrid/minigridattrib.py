...
class WorldObj:
    ...
    def __init__(self, type, color, shade, size):
...
class Wall(WorldObj):
    def __init__(self, color='grey', shade=IDX_TO_SHADE[0], size=IDX_TO_SIZE[0]):
        super().__init__('wall', color, shade, size)
...
class Key(WorldObj):
    def __init__(self, color, shade, size):
        super(Key, self).__init__('key', color, shade, size)
...
class Ball(WorldObj):
    def __init__(self, color, shade, size):
        super(Ball, self).__init__('ball', color, shade, size)
    ...
    def decode(array):
        ...
        for i in range(width):
            for j in range(height):
                ...
                size = IDX_TO_SIZE[sizeIdx]
                # State, 0: open, 1: closed, 2: locked
                is_open = state == 0
                is_locked = state == 2

                if objType == 'wall':
                    v = Wall(color, shade, size)
                elif objType == 'floor':
                    v = Floor(color)
                elif objType == 'ball':
                    v = Ball(color, shade, size)
                elif objType == 'key':
                    v = Key(color, shade, size)
                elif objType == 'box':
                    v = Box(color)
                elif objType == 'door':
