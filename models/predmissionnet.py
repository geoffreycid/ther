...
class PredMissionOneHot(nn.Module):
    ...
    def optimize_model(self, memory, config):
        ...
        len_memory = len(memory.memory)
        len_memory_train = int(len_memory * 0.9)
        len_test = len_memory - len_memory_train
        train_memory = memory.memory[:len_memory_train]
        test_memory = memory.memory[len_memory_train:]

        if self.use_dense:
            len_memory_dense = len(memory.memory_dense)
            len_memory_train_dense = int(len_memory_dense * 0.9)
            len_test_dense = len_memory_dense - len_memory_train_dense

            train_memory_dense = memory.memory[:len_memory_train_dense]
            test_memory_dense = memory.memory[len_memory_train_dense:]
        ...
        while True:
            ...
            batch_type_predictions, batch_color_predictions, batch_seniority_predictions, batch_size_predictions = self(
                batch_states)

            # Balanced sample from the dataset
            if self.use_dense:
            ...
            if self.use_dense:
                dense_loss = self.criterion(batch_dense_predictions, batch_dense_targets)
                loss = sum(
                    [type_loss, color_loss, seniority_loss, size_loss, dense_loss * config["ratio_dense_onehot"]])
            else:
            ...
            if steps_done % config["log_every"]:
                ...
                if self.use_dense:
                    ...
                    batch_states_dense = torch.cat(batch_imcs_dense.state)
                    batch_dense_predictions = self.prediction_dense(batch_states_dense)
                ...
                # Predictions
                batch_mission_predictions = self.prediction_mission(batch_states)
