...
def optimization(train_memory, test_memory, config, train_memory_dense, test_memory_dense):
    ...
    dir_save_model = config["dir"] + "/saved_model/"

    if use_imc:
        dir_logs = dir_logs + "imc" + "/" + "len_train_{}".format(len_train)
        dir_save_model = dir_save_model + "imc" + "/" + "len_train_{}".format(len_train)
    if use_onehot and not use_dense:
        dir_logs = dir_logs + "onehot" + "/" + "len_train_{}".format(len_train)
        dir_save_model = dir_save_model + "onehot" + "/" + "len_train_{}".format(len_train)
    if use_onehot and use_dense:
        dir_logs = dir_logs + "dense" + "/" + "len_train_{}_len_dense_{}".format(len_train, len_train_dense)
        dir_save_model = dir_save_model + "dense" + "/" + "len_train_{}_len_dense_{}".format(len_train, len_train_dense)
    ...
    keep_frames = (config["frames"] - 1) * config["channels"]
    ...
    while True:
        ...
        if use_dense:
            ...
            batch_states_dense = torch.cat(batch_imcs_dense.state)[:, keep_frames:].to(device)
        ...
        batch_states = batch_states[:, keep_frames:].to(device)
        ...
        if steps_done % config["log_every"] == 0:
            if use_imc:
                ...
                batch_states = torch.cat(batch_imcs.state)[:, keep_frames:].to(device)
                batch_missions = torch.cat(batch_imcs.mission).to(device)
                ...
                if use_dense:
                    ...
                    # Keep only the last frame
                    batch_states_dense = batch_states_dense[:, keep_frames:].to(device)
        ...
        if steps_done % config["save_every"] == 0:
            path_to_save = os.path.join(dir_save_model, "{}_steps_{}_acc.pt".format(steps_done, test_accs[-1]))
